...
class LCD_1inch8(lcdconfig.RaspberryPi):
    ...
    def SetWidgets(self, Xstart, Ystart, Xend, Yend):
    ...
    def clear(self, color=0XFFFF):
        ...
        if (self.LCD_Scan_Dir == L2R_U2D) or (self.LCD_Scan_Dir == L2R_D2U) or (self.LCD_Scan_Dir == R2L_U2D) or (self.LCD_Scan_Dir == R2L_D2U) :
            ...
            self.SetWidgets( 0 , 0 , LCD_X_MAXPIXEL , LCD_Y_MAXPIXEL  )
            self.digital_write(self.DC_PIN,self.GPIO.HIGH)
            ...
            self.SetWidgets( 0 , 0 , LCD_Y_MAXPIXEL , LCD_X_MAXPIXEL  )
            self.digital_write(self.DC_PIN,self.GPIO.HIGH)
            for i in range(0,len(_buffer),4096):
                self.spi_writebyte(_buffer[i:i+4096])    
    ...
    def ShowImage(self,Image):
        ...
        self.SetWidgets ( 0, 0, self.width, self.height)
        self.digital_write(self.DC_PIN,self.GPIO.HIGH)
        for i in range(0,len(pix),4096):
            self.spi_writebyte(pix[i:i+4096])	
        '''
        self.SetWidgets ( Xstart, Ystart, self.LCD_Dis_Column , self.LCD_Dis_Page  )
        self.digital_write(self.DC_PIN,self.GPIO.HIGH)
        # Pixels = Image.load()
